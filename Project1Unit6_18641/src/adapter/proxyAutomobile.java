...
public abstract class proxyAutomobile {
     ...
     */
    private static LinkedHashMap<String, Automobile> autoSet = new LinkedHashMap<String, Automobile>();
    ...
    // public void updateOptionPrice(String modleName, String optionSetName,
    ...
    public Automobile acceptPropertiesFromClient(Properties properties) {
        ...
        if (automobile != null)
            autoSet.put(automobile.getModel(), automobile);
        return automobile;
    }

    public String listAutomobileNames() {
        StringBuilder builder = new StringBuilder();
        for (Automobile auto : autoSet.values()) {
            if (builder.length() != 0)
                builder.append("\n");
    ...
    public ArrayList<Automobile> getModelList() {
        ArrayList<Automobile> modelList = null;
        for (Automobile auto : autoSet.values()) {
            modelList.add(auto);
        }
        return modelList;
    ...
    public void buildDBAuto(String filename) {
        FileIO io = new FileIO();
        a1 = io.buildAuto(filename);
        if (autoSet.get(a1.getName()) != null) {
            return;
        } else {
            autoSet.put(a1.getName(), a1);
            int[] aIDs = new DatabaseIO().addToDatabase(a1, AutoID, OptionSetID, OptionID);
            AutoID++;
            OptionSetID = aIDs[0];
            OptionID = aIDs[1];
        }
    ...
    public void deleteDBAuto(String automobileName) {
        ...
        if(autoSet.size() == 0) {
            System.out.println("Error: There is no automobile in database");
        }
        boolean found = false;
        for(int i = 0; i < autoSet.size(); i++) {
            if(autoSet.containsKey(automobileName)) {
                found = true;
                autoSet.remove(automobileName);
                databaseIO.deleteAutomobileInDatabase(automobileName);
            }
        }
        if(!found) {
            System.out.println("Not found automobile " + automobileName + "in database");
        }
        
    }

    public void updateDBAutoPrice(String automobileName, int newBasePrice) {
        DatabaseIO databaseIO = new DatabaseIO();
        Automobile auto = autoSet.get(automobileName);
        if(auto != null) {
            auto.setBasePrice(newBasePrice);
            databaseIO.updateAutomobileBasePrice(automobileName, newBasePrice);
        } else {
            System.out.println("Not found automobile " + automobileName + " in the database.");
        }
    }
